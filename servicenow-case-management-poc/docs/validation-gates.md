...
- **Superseded on 2026-09-02: the shipping package is now 988 blocks / 4,062,436 bytes / `90ee0249…` (the
  native-rebuild package), and the full trip HAS been measured on its 988 payload records — 0 `type=error` and
  0 `type=warning` preview problems on a genuinely clean instance, then a single UI-action commit that
  succeeded 100%. See [`refine-run/FINAL-REPORT.md`](./refine-run/FINAL-REPORT.md). The post-review CR1
  re-sequencing later reordered the `sys_update_xml` blocks into AAP §0.5.2 dependency order and changed
  nothing else — every payload body and identity is byte-identical to the previewed bytes (block-multiset
  proof), the byte count and the 44-class census are unchanged — so the digest above is the reordered file's
  while the live evidence attaches to the same records; the reordered bytes were verified statically and were
  not themselves re-imported. The 926-block / 3,781,097-byte / `7292a6fe…`
...
- **Status of this procedure:** it has been executed end-to-end on the 913-block / 3,618,378-byte /
  ...
  **Superseded on 2026-09-02: on the shipping package's 988 records — 988 blocks / `90ee0249…` — steps 1-4
  plus the commit were executed and reached 0 problems of any type followed by `state=committed`
  ([`refine-run/FINAL-REPORT.md`](./refine-run/FINAL-REPORT.md)); the post-review CR1 re-sequencing changed
  only the block order afterwards, leaving those records byte-identical, so no step has to be re-run on the
  reordered file, which was verified statically instead. Not even steps 1-4 were ever run on the
  926-block / `7292a6fe…` bytes now retained as `…FALLBACK.xml`;** §0.3c of the
