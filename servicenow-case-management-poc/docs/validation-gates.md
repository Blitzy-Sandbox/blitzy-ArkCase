...
- **Superseded on 2026-09-02, and it splits in two — read both halves.** *(1) What was verified live:* the full
  ...
  [`refine-run/FINAL-REPORT.md`](./refine-run/FINAL-REPORT.md). *(2) What ships:* the post-review CR1
  re-sequencing put those same 988 records into AAP §0.5.2 dependency order, so the file on disk today is
  **988 blocks / 4,062,436 bytes / `90ee0249…`** — that is the digest to verify the artifact against, and
  **the exact-byte round trip on it is owed and has not been run.** Under this run's frozen rule — if the
...
- **Status of this procedure:** it has been executed end-to-end on the 913-block / 3,618,378-byte /
  ...
  **Superseded on 2026-09-02, in two parts. On the 988 records as they stood in the pre-reorder,
  previewed-and-committed byte sequence — 988 blocks / 4,062,436 bytes / `eee9fabd…` — steps 1-4 plus the
  commit were executed and reached 0 problems of any type followed by `state=committed`
  ...
  steps 1-8 have NOT been executed on that byte sequence — they are owed on it, and running §5 of
  [`HUMAN_DEPLOYMENT_RECREATE_GUIDE.md`](./HUMAN_DEPLOYMENT_RECREATE_GUIDE.md) against the `90ee0249…` file
  on a genuinely clean PDI is what discharges them. The reordered file was checked statically instead
