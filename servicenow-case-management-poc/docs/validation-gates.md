...
- **Superseded on 2026-09-02, then amended on 2026-09-03 by the choice-composite fix and again by the three
  ...
  *(3) Where a gate was run on a candidate — the 2026-09-08 revision `b2217224…`, by a same-instance
  ...
  shipping 2,994,341-byte `751ceb61…` file has never been uploaded, previewed or committed:* the Update Set consolidation rebuilt the
...
> revision's, not the shipping `751ceb61…` file's — 5 gates pass outright · ~~1 passes with a documented post-commit native
...
> that ship** — the shipping 522-block / 2,994,341-byte / `751ceb61…` file has never been uploaded, previewed
...
> **[CR3 2026-09-09 · F13 — NOT A TARGET: that file was deleted on 2026-09-08, is not on disk, and there is no promotion to perform. The only upload, verification or promotion target is the canonical path, `update-set/x_casemgmt_case_management_update_set.xml` — 522 children, SHA-256 `751ceb61…`.]**
...
> skipped, and those bytes are superseded by the ungated 2,994,341-byte `751ceb61…` file.** The two candidate packages named in the retained lines were superseded and **deleted** in this
...
> the shipping 2,994,341-byte `751ceb61…` bytes at all**: the five passes and the ACL schema half were
...
> **no suite run covers the shipping bytes** (`751ceb61…`).
...
- **Status of this procedure:** it has been executed end-to-end on the 913-block / 3,618,378-byte /
  ...
  **Superseded on 2026-09-02, in three parts, and this procedure either has been executed on a byte sequence or
  ...
  it names are superseded — the file on disk is now 522 blocks / 2,994,341 bytes / `751ceb61…`, never
