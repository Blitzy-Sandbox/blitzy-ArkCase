...
- **Superseded on 2026-09-02: the shipping bytes are now 988 blocks / 4,062,436 bytes / `eee9fabd…` (the
  native-rebuild package), and the full trip HAS been measured on them — 0 `type=error` and 0 `type=warning`
  preview problems on a genuinely clean instance, then a single UI-action commit that succeeded 100%. See
  [`refine-run/FINAL-REPORT.md`](./refine-run/FINAL-REPORT.md). The 926-block / 3,781,097-byte / `7292a6fe…`
...
- **Status of this procedure:** it has been executed end-to-end on the 913-block / 3,618,378-byte /
  ...
  **Superseded on 2026-09-02: on the shipping bytes — 988 blocks / `eee9fabd…` — steps 1-4 plus the commit
  were executed and reached 0 problems of any type followed by `state=committed`
  ([`refine-run/FINAL-REPORT.md`](./refine-run/FINAL-REPORT.md)). Not even steps 1-4 were ever run on the
