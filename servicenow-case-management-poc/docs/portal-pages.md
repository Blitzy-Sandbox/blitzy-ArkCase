...
| Stale-message hygiene | Every input carries `ng-change="c.onInputChange()"`, which drops the previous attempt's outcome — the submission's generic banner, refusal summary and per-field server messages, and on the lookup page the result panel, the not-found alert and the service-failure alert together. It fires only on an **actual model change**: the submission widget snapshots the five values when it raises an outcome and clears that outcome only once one of them differs, because `ng-change` also fires when AngularJS trims a keystroke back to the value already in the model, and that is how a whitespace press previously removed the summary while the per-field errors stayed (Delta QA4 Issue 14). A server field message about an **unedited** field survives an edit to a different field, since it is still a true statement. On the lookup page the refusal summary is likewise **not** dropped by an edit that leaves the field empty — typing a character and backspacing it used to remove it while the field's own message stayed beside a still-empty field — so `c.onInputChange()` there returns before clearing it while `c.clientProblemRemains()` (an empty or whitespace-only case number) holds. The lookup needs no snapshot: its single control is a plain `type="text"` input with no native validators, so `ng-change` cannot fire without the model moving. Per-field CLIENT messages are deliberately not cleared here: they are bound to live `$invalid && c.pressed` and correct themselves | both widgets |
...
| Programmatic invalid state | Every control has a `name` and binds **`aria-invalid`** to `$invalid && c.pressed` — the control's own validity **and** the primary button having been pressed at least once — so assistive technology is told *which* control is wrong rather than only that the form is, and a field nobody has acted on yet is not announced as an error on first paint. The second half was each control's `$touched` flag until Delta QA4 Issue 14 (submission) and Issue I9 (lookup): `$touched` flips on **blur**, and the blur caused by the press itself inserted the error paragraph above the button, moved the button, and swallowed the press — see [The first press of the button used to be swallowed](#the-first-press-of-the-button-used-to-be-swallowed) | both widgets |
...
| Per-field error message | Each control has its own paragraph carrying the theme's **`alert-danger`** colour token — applied *without* the `alert` class, so it contributes the colour and no box — and **`role="alert"`**, referenced by the control through `aria-describedby`. It renders while that control is invalid **and the button has been pressed** (`c.clientInvalid(control)` in both widgets' client scripts), **and** — submission page only — while the endpoint has named that field in an HTTP 400 `fields` map. The lookup page has no server field-error channel: its endpoint answers 404 with the verbatim sentence and never a `fields` map, so `c.clientInvalid()` is the whole condition there. The exact strings are in the table below | both widgets |
...
| Control | Message shown when the client considers it invalid **and the primary button has been pressed** (`$invalid && c.pressed`) |
| --- | --- |
...
press was received and refused. Both are cleared by a successful press, and by an edit that actually changes a
value **and leaves nothing the widget would still refuse** — see
[One email rule, and the endpoint's field errors on the page](#one-email-rule-and-the-endpoints-field-errors-on-the-page)
for why "actually" is load-bearing on the submission page, and
[The first press of the button used to be swallowed](#the-first-press-of-the-button-used-to-be-swallowed) for the
lookup page's `c.clientProblemRemains()` guard, which keeps the summary on screen while the case-number field is
still empty.
...
#### The first press of the button used to be swallowed

**The defect, as measured on the rendered public page `x_casemgmt_case_status`.** Focusing the empty **Case Number**
field and tabbing away — or typing a character and backspacing it — set AngularJS's `$touched` on the control. The
field's error paragraph was keyed to `$invalid && $touched`, so that alone rendered `Enter a case number to look
up.`, set `aria-invalid="true"`, and pushed the **Look Up Status** button **20px down**, all *before any submit*. A
real pointer press aimed at the button's original centre then landed on the `<form>` element the button had just
vacated: **click handlers 0, submits 0, `GET /api/x_casemgmt/case_status_lookup` 0, no summary.** A second press at
the button's new position worked. The requester's **first** press on this page's primary public action was therefore
silently swallowed — and the blur that armed the trap is the one the press itself causes, so the sequence was
`mousedown → blur → $setTouched → paragraph inserted above the button → +20px reflow → mouseup on the form → ng-submit
never fires`.

The submission page had the identical defect and it was fixed there first (Delta QA4 Issue 14). This is the lookup
half, Issue I9, fixed the same way so the two public pages behave identically.

**The fix.** Reveal is keyed to the **press**, not to the blur:

| Change | Where |
| --- | --- |
| `c.pressed` — false at construction, set as the **first statement** of `c.lookup()`, before the in-flight guard and before the empty-field branch | client script |
| `c.clientInvalid(control)` = `control && control.$invalid && c.pressed` — one expression behind all three of the field's invalid affordances | client script |
| The `has-error` `ng-class`, the `ng-attr-aria-invalid` binding, the error paragraph's `ng-if`, and the hint's empty-state suppression all read `c.clientInvalid(caseLookupForm.caseNumber)` where they read `$invalid && $touched` | template |
| `c.clientProblemRemains()` = an empty or whitespace-only `c.caseNumber`, tested exactly as `c.lookup()` tests it; `c.onInputChange()` returns before clearing the refusal summary while it holds | client script |
| `c.markAllTouched(form)` is **kept**, but is no longer what reveals anything — it only keeps AngularJS's own `$touched`/`ng-touched` bookkeeping consistent with the press | client script |

Blur now changes nothing on screen — validity itself is already current, because AngularJS revalidates on input and
not on blur — so no layout moves between `mousedown` and `mouseup` and the press is always delivered. After that
first press the flag stays true, so the message tracks the model live: it clears as the field is filled and
reappears if it is emptied, and by then the reflow happens on a keystroke rather than under the pointer.

The one-time prefill path (`?number=…`, step 0 above) goes through `c.lookup()` and so sets `c.pressed` without a
press. That is harmless and correct: the field is populated in that path, so nothing is revealed, and the reveal it
gates can only move layout on a later keystroke.

**No snapshot comparison is needed here, and its absence is deliberate.** The submission widget compares the model
against a snapshot because its `ng-change` can fire *without* the model moving (AngularJS trims the view value
before `$setViewValue`, and `$commitViewValue` skips its unchanged-value early return for a control with **native**
validators, which `type="email"` has). The lookup's single control is a plain `type="text"` input with no native
validators, so both the input directive's own `ctrl.$viewValue !== value` test and `$commitViewValue`'s early return
hold: a space typed into an empty field commits nothing and never reaches `c.onInputChange()`.

**What the fix deliberately does not touch.** No CSS, no class name and no colour changed — reserving the vertical
space would also have closed the defect and is barred by AAP §0.4.4 (*"default theme. No custom CSS, no custom
branding"*), so this widget's `css` element is still empty and the only markup edit keeps `class="form-group"` and
the `has-error` name exactly as they were. The verbatim `No case found with that number.` sentence and the
three exposed fields (`status`, `subject`, `opened_date`) are unchanged, as AAP §0.7.4 fixes both. None of the
[Open accessibility limitations](#open-accessibility-limitations-blocked-by-aap-044--032) A-1 to A-6 is closed or
altered by this fix: they are colour-token, target-size, viewport and heading items, and all six remain **OPEN**
exactly as recorded.

**Verified without contacting the instance**, because the live record and the runtime re-verification are applied by
the engineer who owns this checkpoint: `xmllint --noout` on the widget XML passes; `node --check` on the
`client_script` and `script` CDATA passes; and the extracted controller was driven against stubbed `$http`/`$scope`
through 46 assertions, all passing — pristine state reveals nothing, `$setTouched` alone still reveals nothing (the
swallowed-press precondition is gone), the first press sets `c.pressed`, renders the field message, sets the summary
and issues **no** request, an edit back to empty keeps the summary while an edit to a real number withdraws it, a
valid press issues exactly one `GET` with the 20-second deadline and whitelists exactly `status`/`subject`/
`opened_date`, 404 and 5xx still split into the not-found and service-failure panels, and the prefill path issues
exactly one read.
