...
2. **Status: GATE-VERIFIED on these exact bytes, 2026-09-09 (code review CR5).** *This item read
   ...
   uploaded, previewed to 0 problems of any type and committed once on 2026-09-09, by a same-instance
   reset-and-reimport. The suite in this document ran against **that** commit — which is what makes item 9 a
   result on the shipping bytes rather than on an earlier install.
3. **A commit of these bytes was reported clean by the platform (code review CR5).** *Retained as written,
   and describing the earlier attempt only:* "The one gate this project ran did not run on these bytes, and
   the platform did not report it clean. It ran on 2026-09-08 against a superseded **3,114,377**-byte revision
   (SHA-256 `b2217224…`, also 522 blocks): the preview reached **0 `type=error` and 0 `type=warning`**, but
   the platform's own verdict on the single native commit was **"Failed at 100% — the update set commit
   completed but some updates failed to commit"**, with **three** `sys_user_has_role` rows skipped
   (`permission denied: no thrown error`). **No candidate has yet produced a commit the platform reported as
   clean**, so a "GATE MET" or "GATED" label anywhere below describes that attempt and not a clean pass."
   **CR5 2026-09-09:** the 2026-09-09 commit of the item-1 bytes returned **`Succeeded 100%`** / **`Update set
   committed - Succeeded in 40 Seconds`**.
4. **A fresh suite result and a fresh harness result now cover the shipping bytes — see item 9 (code review
   CR5, finding F04).** *This item read "**No test result covers the shipping bytes.** Neither the 20-test /
   180-step ATF suite nor the 13-assertion transition harness has been run against them. The most recent suite
   result — **`TES0001006`**, created **2026-09-08 22:09:18 UTC**, 20 tests = **4 Success / 16 Failure / 0
   Error / 0 Skipped** — and the most recent harness pass (`TOTAL=13 PASSED=13 FAILED=0`, 2026-09-08 22:17:27
   UTC) both ran against the artifacts the gated revision's commit created."* Both stand as dated provenance
   of that revision.
...
8. **Why item 1's identity differs from the Step 5c export it came from — code review CR4, findings F02 and
   ...
   **[CR5 2026-09-09 · F04 — the last two sentences are SUPERSEDED.** What this item records about the
   ...
   no test coverage. Coverage was created by **running the suite**: these bytes were committed on 2026-09-09
   and both the suite and the harness were then run against that commit — item 9. The package remains ungated
...
9. **CURRENT TEST RESULT — the suite and the harness, run against a commit of these exact bytes, 2026-09-09
   ...
   | | |
   | --- | --- |
   | Suite result | **`TES0001007`** (`sys_atf_test_suite_result` `sys_id` `2f50a71493df8b1009aa70d19dba1090`) |
   ...
   **All twenty passed**, by area: `ATF 01` (the §0.5.7 data model) · `ATF 02`-`ATF 07` (the RBAC matrix,
   including assigned-only agent access and the task/party mirror) · `ATF 08`-`ATF 14` (every state
   transition and both prohibited transitions) · `ATF 15`-`ATF 17` (the form-level blocking behaviour) ·
   `ATF 18`-`ATF 20` (the anonymous portal contract). No failing step exists in this result, so there is no
   failure message to transcribe.
   ...
   **THE KNOWN-FAILURE REGISTER, CORRECTED. All three of the failures this project recorded as known passed
   ...
   | --- | --- | --- |
   | **`ATF 17`** | `Unable to set field 'status' to value 'In Progress'. Field 'status' is not editable` — the application's own form lock on Closed cases defeated the test's technique. Recorded as a known failure of `TES0001005` (17 Success / 3 Failure), 2026-09-08 | ✅ **PASS** in `TES0001007`. Did not recur |
   | **`ATF 18`** | The `opened_date` character-for-character assertion failed on a **7-hour offset** — the table stored `…17:39:40` while the endpoint returned `…10:39:40`, the portal labelling the value "Opened Date (America/Los_Angeles)". Recorded as a known failure of `TES0001005`, 2026-09-08, and flagged as an unratified spec point | ✅ **PASS** in `TES0001007`. Did not recur |
   | **`ATF 19`** | The same `opened_date` offset assertion, on the lookup leg. Recorded as a known failure of `TES0001005`, 2026-09-08 | ✅ **PASS** in `TES0001007`. Did not recur. The lookup of `CASE9000002` returned `opened_date` `2026-09-08 18:58:04`, matching the stored value |
   | **`TES0001006`'s sixteen failures** (2026-09-08 22:09:18 UTC, 4 Success / 16 Failure) | One root cause, not sixteen defects: the three demo personas held **no role grants**, so every impersonation-based assertion measured the absence of a grant rather than the application | ✅ **None recurred.** The §5h grant step ran before this suite, exactly as the deployment guide now instructs |
...
>
> ⛔ **NOT A SUPPORTED STEP — CR3 2026-09-09 · F16.** **This callout, and prerequisite P1b below, require a Global-scope remediation run before the
> suite's results may be interpreted.** It violates AAP §0.7.2's zero-global-write constraint and the single-clean-commit gate ("no second commit, no remediation script, no live-instance patching"); it is retained only as a record of what an earlier round did. A shortfall the package leaves is a source-side defect: correct the package where it is produced and re-run the full gate on the exact candidate bytes — see SUPPORTED INSTALL ROUTE at the top of this document. What replaces it: interpret the suite only after a
> **single clean commit of the exact candidate package**, which is what delivers the physical schema, the 24
> choice values and the 26 ACLs with their 27 `sys_security_acl_role` links. The callout is retained below as
> the record of the superseded hand-authored candidates, on which it was true.
>
...
> fails with `invalid table name` and the whole of Scenario A collapses) and leaves the package's **26** ACLs with **0 of their
> 27 `sys_security_acl_role` link rows** (this sentence read "the 29 ACLs"; 29 counts the repository's `acl/*.xml` artifact set, not the package — corrected 2026-09-09, CR3 F18) (so on a high-security instance every ACL evaluates to *deny* and the
...
| P4 | **A browser-attached client runner — required, not optional, on this instance** | `sn_atf.headless.enabled` is **`false` on the current validation instance `dev306625`** — read over the Table API on 2026-09-03, with `sn_atf.runner.enabled` already `true` there — and it was `false` on the retired `dev379024` too, where it could not be enabled, so client-side steps cannot run unattended. Open `/atf_test_runner.do?sysparm_nostack=true` in a second browser tab **before** launching the suite, leave it open, then press **Run Test** and select that browser under "Pick a Browser". The three form-level tests (`ATF 15`-`17`) need it; the server-side tests do not, but the suite as a whole will stall waiting for a runner if one is not registered. |
...
> The shipped `tables/*.xml` now carry exactly that two-open/three-closed pattern, which is what the commit
> delivers. `scripts/post_import_remediation.js` reconciles all eight columns — opening what must be open and
...
shows it. The defect it exposed has since been fixed at its root cause (M5) and the test passed when last measured on a
remediated instance — not a current verdict: in the most recent run, `TES0001006` of 2026-09-08, `ATF 07` is one
