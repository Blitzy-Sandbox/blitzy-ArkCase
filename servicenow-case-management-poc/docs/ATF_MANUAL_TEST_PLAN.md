...
| # | Prerequisite | How |
| --- | --- | --- |
| P1 | The application is installed (3 tables, 3 roles, ACLs, business rules, script includes, 2 scripted REST services) | Commit `update-set/x_casemgmt_case_management_update_set.xml` |
| P2 | **`sn_atf.runner.enabled` = `true`** | `All → System Properties → All Properties`, filter `sn_atf.runner.enabled`. **This is an instance setting, not an application artifact — it is deliberately NOT in the Update Set.** Without it every run aborts. |
| P3 | Demo identities exist | `x_casemgmt_demo_manager`, `x_casemgmt_demo_agent`, `x_casemgmt_demo_viewer`, group `x_casemgmt_demo_team` (seeded by the Update Set; also creatable with `scripts/seed_demo_data.js`) |
| P4 | For the three **form-level** tests only: a client test runner | Open `/atf_test_runner.do?sysparm_nostack=true` in a second browser tab, leave it open, then press **Run Test** and pick that browser. Server-side-only tests need no browser. |
| P5 | Optional — headless runs | `sn_atf.headless.enabled` is `false` on this instance and was **not** changed. Enable it only if you want unattended client runs, and disclose it. |
...
| F11 | `Send REST Request - Inbound` supports only `basic` and `mutual` auth, and its *Query parameters* are **static**. | It cannot exercise a genuinely anonymous caller, and it cannot read a number produced earlier in the same test. Hence: a `Run Server Side Script` companion using `sn_ws.RESTMessageV2` with no credentials for the anonymous path, and a **pinned, out-of-sequence fixture number `CASE9000019`** for the valid-lookup test so it stays portable to a freshly imported instance whose counter restarts at `CASE0000001`. |
| F12 | ATF rolls back records created by its own steps **and** by its script steps — but **not** records created by an HTTP request *into* the instance (that runs as `guest`, in its own transaction). It also reverses the test's own cleanup delete. | The anonymous-submit test leaves exactly one synthetic case per run. Clean up with: delete `x_casemgmt_case` where `subject` **starts with** `ATF-PORTAL-18`. |
...
> **This test is currently RED on the shipped suite, and correctly so.** It asserts the AAP §0.5.6 mirror; the application's four scripted child-table ACLs dereference `current.case` (F4) and therefore fail to compile, which makes the agent narrowing deny *every* row. See §8. The test turns green with no edit once the ACL is fixed.
...
| 9 | Run Server Side Script | look the case up by that subject and assert `status==='Draft'` and `number` matches `/^CASE\d{7}$/` |
| 10 | Run Server Side Script | delete the created case (and note F12: the rollback reverses this delete, so one row survives each run — clean it up afterwards) |
...
`Send REST Request - Inbound` authenticates as the test executor (F11), so it does **not** prove the anonymous contract. Run this from any shell, with **no credentials at all**, and record the output alongside the suite:
...
# expect 401 — the scoped Table API is closed to anonymous callers by design (ws_access=false)
```

Then delete the two `ATF-CURL-COMPANION` / `ATF-PORTAL-18` cases the anonymous calls created (F12).
...
7. After `ATF 18` runs, delete `x_casemgmt_case` where `subject` starts with `ATF-PORTAL-18` (F12).
