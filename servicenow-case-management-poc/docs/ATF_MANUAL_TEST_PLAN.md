...
| Folded into the package | **761 of the package's 913** `<sys_update_xml>` blocks in `update-set/x_casemgmt_case_management_update_set.xml` (3,643,389 bytes, SHA-256 `89638c17…`) — the suite is the single largest thing in the deliverable. The 761-block ATF range is byte-unchanged by the QA-remediation pass, which touched 9 non-ATF payloads |
