...
> `<sys_update_xml>` blocks**. The round trip on that exact sequence was never run, so it is retained, **not
> shipped**, at `update-set/x_casemgmt_case_management_update_set.REBUILT-DEPENDENCY-ORDERED.xml`. It satisfies
...
> re-previewed or re-committed anywhere; that is corroboration, not a round trip of that exact byte sequence.
> The record is
...
> settled which package ships; it did not pass the gate.** §5 of that guide, run against the elected file on a
...
>
> **The elected bytes are 926 blocks, 3,781,097 bytes, SHA-256
...
- **Two deliberately named artifacts are retained alongside it and MUST NOT be deleted.** They are not deliverables and no deployer imports them; each is on disk for a documented reason:
  - `x_casemgmt_case_management_update_set.FALLBACK.xml` — the retained original, byte-identical to the shipping deliverable (926 blocks, `7292a6fe…`). Its retention is **required** by directive D3/S0: the elected fallback must stay on disk under its own name so the bytes that were elected remain independently identifiable even after a later export overwrites the deliverable path.
  - `x_casemgmt_case_management_update_set.REBUILT-DEPENDENCY-ORDERED.xml` — the AAP Section 0.5.2 dependency-ordered rebuild (988 blocks, `90ee0249…`), retained but **not shipped**. It is the upgrade path described in the note under *Purpose* above and in §10.0 of [`PDI_LIMITATIONS_AND_KNOWN_ISSUES.md`](./PDI_LIMITATIONS_AND_KNOWN_ISSUES.md): once the full gate has been run on those exact bytes it can be promoted onto the deliverable path.
...
> lists** are outstanding on either package: `sys_choice` rows are absent for the
...
> `7292a6fe30413a9fb0b115e160c668edb7487b4391865b21a011a7be1add66b7`, because no upload → preview → commit
> trip of any kind has been run on those bytes (the note under *Purpose* above, and §10.0 of
