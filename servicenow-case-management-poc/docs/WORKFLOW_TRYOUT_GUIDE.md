...
> `https://devXXXXXX.service-now.com` and see the case lifecycle, role-based access, the external portal, and
> the dashboards in action.
>
> **Instance note.** `devXXXXXX` is the reachable instance, running **Zurich Patch 10**; its Table API answered
...
1. Sign in to `https://devXXXXXX.service-now.com` as **`admin`**.
...
**`https://devXXXXXX.service-now.com/x_casemgmt_case_portal`**
...
```bash
SN="${SERVICENOW_INSTANCE_URL:-https://devXXXXXX.service-now.com}"   # export SERVICENOW_INSTANCE_URL, or replace the devXXXXXX placeholder with your own PDI host
