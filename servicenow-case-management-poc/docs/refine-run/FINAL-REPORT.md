...
delivery election in part (d) does (the bytes on the deliverable path today are that untouched
package itself — `7292a6fe…`, 926 blocks, 3,781,097 bytes, `cmp`-identical to `…FALLBACK.xml`. It
held that base **as amended** by three later commits, `9f3ea74c…` / 935 / 3,973,569, until remedy
(a) of D48's stop condition was executed; those amended bytes are retained, explicitly
non-shipping, at `…AMENDED-NOT-GATED.xml`. See the RESTATED section at the top).
...
| Item | Value |
| --- | --- |
| **Deliverable path** | `servicenow-case-management-poc/update-set/x_casemgmt_case_management_update_set.xml` — it holds the **exact, untouched elected fallback**, and `cmp` against `update-set/x_casemgmt_case_management_update_set.FALLBACK.xml` reports **no difference**. **CORRECTED:** this row said it held that base *as amended* and was *not* byte-identical to the fallback, which was true of the path from commit `f8454fb078` until **remedy (a) was executed**; the amended bytes are retained, explicitly non-shipping, at `…AMENDED-NOT-GATED.xml` |
...
| Size / payloads | **3,781,097** bytes · **926** `<sys_update_xml action="INSERT_OR_UPDATE">` blocks · `xmllint --noout` clean · 0 duplicate update names · 0 payloads missing `<name>`, `<type>` or `<payload>` · one descriptor block · all actions `INSERT_OR_UPDATE`. **CORRECTED:** this row read **3,973,569** bytes · **935** blocks as measured 2026-09-05T04:45Z, which are the figures of the retained amended package at `…AMENDED-NOT-GATED.xml` and were the deliverable's while those bytes sat at its path |
| **Provenance, and where the amendment went** | Elected base `7292a6fe…` (926 payloads) at commit `3671901b5b`, then `f8454fb078` (choice materialization and seed references) → `6efb13b141` (18 QA findings) → `8dfdbcb015` (independent-verification remediation), which took the path to `9f3ea74c…` (935 payloads). Delta of that amended package against the base: **+16 / −7 = +9** payload names, **919** in common — 4 business rules, 1 client script, 3 field-level ACLs and 1 form-layout record added, and the 7 `sys_choice` payloads renamed from random-32-hex to `sys_choice_x_casemgmt_*`. **CORRECTED:** this row said the deliverable was *not* reverted to the base. It has since been: **remedy (a) was executed**, the recorded bytes are back at the deliverable path, and the +9 amendments are retained — not deleted — at `…AMENDED-NOT-GATED.xml`, because D3's authorised path on an unmet hard gate is the *untouched* package. What the shipped package therefore lacks is itemised at the head of this report |
| **Label on the shipping package** | **It does NOT include this round's native-rebuild fix.** Measured on the shipping file: 926 payload blocks, 3,781,097 bytes, 0 `sys_documentation` rows, 0 `sys_security_acl_role` rows, **26** `sys_security_acl` rows, **7** `sys_script` business rules, 7 older `sys_choice_<32-hex>` rows, 25 hand-authored `sys_dictionary` rows with random-32-hex update names, 3 hand-authored `sys_db_object_<32hex>` table records. **An importer must run `scripts/post_import_remediation.js`** after the commit to create the physical schema and the ACL-role links, exactly as the pre-refine deployment did; that script asserts 29 ACLs / 36 links, the figures the repository's 29 `acl/*.xml` artifacts describe, so on this 26-ACL package it reports the 3-ACL / 9-link shortfall as a **named** non-convergence. **CORRECTED:** this row said the package *does* carry the choice-materialization fix and **29** ACLs. It does not — those belong to the retained amended package, and the absent 7 name-keyed choice collections are the single root cause of the six ATF failures in part (e) |
...
> **⚠ THIS STORED RESULT IS STALE RELATIVE TO CURRENT RUNTIME.** It is **definitive for the run that
> produced it** — the 14/6 split, the per-test verdicts, the failing steps and the verbatim assertion
> text below are the measured record and are preserved intact — and it **no longer describes the live
> instance**. Two live measurements make it stale: **`sys_choice` for the three scoped tables reads
> 24 rows** (it read 0 when the suite ran), and an **anonymous portal
> `POST /api/x_casemgmt/case_submit` returns HTTP 201** with a `CASE…` number, which is exactly the
> assertion ATF 18 recorded as `400`. All six recorded failures — ATF 01, 10, 15, 16, 17 and 18 —
> trace to that one absent-`sys_choice` cause, so **the six no longer describe current behaviour on
> this instance and no verdict below may be read as its current state.** **A suite re-run is owed and
> was deliberately not performed:** nothing was re-run, and **no ATF test, suite, step or result
> record was created, altered or deleted**. Those two figures are recorded with their source — the QA
> gates that exercised this instance after the fix — and were **not** re-measured here, because the
> pass that added this note makes no call to the PDI.
> **What the result does still describe is the package that ships.** The shipped elected fallback
> carries the 7 older `sys_choice_<32-hex>` payloads instead of the 7 name-keyed
> `sys_choice_x_casemgmt_*` collections, so a bare commit of the shipping bytes on a clean instance,
> without `scripts/post_import_remediation.js`, would be expected to reproduce all six. **The
> instance is fixed; the package is not** — and **Phase 3 gates nothing either way** (D4 /
> OVERRIDE-4): the package ships on Phase 2's result regardless of this phase's outcome.
...
2026-09-02 21:45:31Z — *before* the choice-materialization fix landed. **All six of its failures (ATF
01, 10, 15, 16, 17, 18) reduce to the `x_casemgmt_case` choice lists resolving empty at runtime** —
including ATF 18, whose anonymous portal submit returned 400 where 201 was expected, because
`CasePortalService._resolveCaseTypeChoice()` fails closed on an empty choice list. **That cause is
absent from the live instance today: `sys_choice` reads 24 rows and the same anonymous submit returns
HTTP 201. A suite re-run is owed and was deliberately not performed, no ATF record was altered, and
this artifact must not be read as the current state of the application** — only as the record of the
run that produced it, and as an accurate description of the package that ships.
...
choice-typed. Re-verified live **for this report, on 2026-09-03: `sys_choice` rows for the three
tables = 0.** **SUPERSEDED AS A CURRENT-STATE STATEMENT:** the choice composites have since been
committed to this instance and `sys_choice` for the three tables reads **24** rows, so this root
cause is absent from current runtime and present only in the package that ships. The 0-row figure is
retained as the dated measurement the six verdicts rest on.
...
Phase 2's fix loop removed were inside it, taking it to **51** at that point; **CORRECTED — the
canonical figure is 111**, measured at the 2026-09-05 reconciliation, after the nine amendment
payloads and their companion captures landed in this set. **That count is a live snapshot of a
non-shipping local update set, not a settled total:** the platform keeps capturing into whichever
set is current, so unrelated activity can add a child to it at any time and a later, higher reading
is expected rather than contradictory — a subsequent read of **112** was observed, caused by one
further platform-captured record and by nothing this run did. The number is not chased; every
reading is namespaced in [`run-state.json`](./run-state.json) at
`history_snapshots_do_not_read_as_current.absorber_set_child_count_readings`, and nothing in the
delivery turns on it because the set is never shipped) — both created to keep delete-captures out of
the shipping set and never shipped; and, in Phase 2's fix loops, 231 stale
...
still resolve. **Items 6 and 7 were added on 2026-09-05** and are new: item 6 was added because
directive D48's stop condition was **live** on the bytes at the deliverable path, and it is recorded
below as **CLOSED by remedy (a)** — the recorded bytes were put back, so what bears on delivery now is
not the mismatch but its price.
...
commands returned **244** as measured 2026-09-05T07:40Z. **SUPERSEDED AGAIN — the census is 247 tracked and 247 on disk** as measured at commit `3ce969fa49` (`git ls-files servicenow-case-management-poc | wc -l` and `find servicenow-case-management-poc -type f | wc -l` both return **247**; by extension **xml 224** / js 4 / md 18 / json 1). The +3 over the 2026-09-05T07:40Z reading of 244 is `update-set/x_casemgmt_case_management_update_set.AMENDED-NOT-GATED.xml`, `client_scripts/x_casemgmt_case_closed_readonly_enforce.xml` and `ui_policy/x_casemgmt_case_closed_readonly.xml`. Later QA-remediation passes keep adding artifacts, so re-derive it with those two commands rather than trusting the literal — a higher reading is expected and is not a contradiction. That is the figure README's Directory Layout now states. One example named in the refinement brief — the claim
...
The deliverable path therefore **holds the exact, untouched elected base** — `7292a6fe…`, 926
blocks, 3,781,097 bytes, `cmp`-identical to `…FALLBACK.xml` — labeled as not carrying this round's
native-rebuild fix, and **no field or sentence in this run's record designates any artifact on disk
shippable or platform-verified.** *(This sentence read "holds the elected base as amended", which
was true of the path from commit `f8454fb078` until remedy (a) was executed; those amended bytes are
retained, explicitly non-shipping, at `…AMENDED-NOT-GATED.xml`.)*
...
| Phase 3, line 243 — ATF summary | **RE-CAPTURED** 2026-09-05T04:50Z | `…/blitzy/screenshots/qa-refix-phase3-atf-suite-result-recaptured.png` | ATF suite result **TES0001002** (`sys_id 0b7d459a93cf435009aa70d19dba10be`), suite "x_casemgmt Case Management POC": Status **Failure**, Duration 2 Minutes, rolled-up success **14** / failure **6** / error 0 / skip 0, with Test Results (20) enumerated. **Staleness warning:** the stored result dates from 2026-09-02 21:45:31Z, *before* the choice-materialization fix, and **all 6** of its failures reduce to the `x_casemgmt_case` choice lists resolving empty at runtime (ATF 18 included, via the portal service's fail-closed path). That cause is absent from the live instance today — `sys_choice` reads 24 rows and the anonymous portal `POST` returns HTTP 201 — so **the result is STALE RELATIVE TO CURRENT RUNTIME: a suite re-run is owed, was deliberately not performed, and this is not the current state of the application**; it remains an accurate description of the package that ships |
...
2026-09-05. **CORRECTED: no directive checkpoint is missing.** This sentence read "Two checkpoints
are missing and are named as missing above", which was true until the Phase 1 S1 probe table and its
role link were re-captured from a fresh disposable probe; **all five directive checkpoints now
resolve to a readable PNG on disk**, `phase1-s1-probe-table-studio.png` (241,921 bytes) and
`phase1-s1-probe-role-link.png` (142,655 bytes) among them, both confirmed with `test -f` under the
canonical screenshots directory. Supporting (non-
