...
> that ships and `scripts/post_import_remediation.js` must be run to create the 36 links.
...
- **Why the links do not arrive from *this* package — measured, and scoped precisely to the cases each
  ...
  **The proven portable route: platform-captured rows committed through a normal retrieved-set
  ...
  For the elected deliverable the practical position is unchanged: hand-authoring 36
...
>   that directives D2 and D21 ordered is **not in it**, and the 36 links must therefore be created by
