...
> | --- | --- | --- |
> | `update-set/x_casemgmt_case_management_update_set.xml` — **THE DELIVERABLE** | **522** `<sys_update_xml>` blocks · **3,114,377** bytes · SHA-256 **`b2217224888fb9b6de664ae816dcee8748507c37e9da0f2d259cc676cd4105a4`** · 26 `sys_security_acl` + 27 `sys_security_acl_role` · descriptor `sys_id` `8ebb770493534b1009aa70d19dba102a` · `xmllint --noout` clean | **GATE MET on these exact bytes, 2026-09-08** — a genuine platform export (every block carries a `<payload_hash>`), uploaded to an instance torn down to a recorded zero-state (CR2 F06 — throughout this document, a "recorded zero-state" is the pre-commit (Step 5b) teardown whose ten checks were run and whose results were transcribed, but whose verbatim request-and-body captures are not retained; it is reported as recorded at the time, not proven, and the gate row at §0.3 states which half of the verdict basis is evidenced), previewed to 0 `type=error` and 0 `type=warning`, then committed once through the native UI action. A **same-instance reset-and-reimport**, not an independent second PDI |
...
> 3,114,377 bytes / `b2217224…`.** It was produced by the platform's own application-publish path on an
...
> before the Update Set consolidation.** AAP §0.7.1 / Gate 7 is **MET on the bytes that ship now, on a
> pre-commit zero-state precondition that is evidence-unproven at raw level** — the split of that basis
...
What ships is the consolidated platform export at `update-set/x_casemgmt_case_management_update_set.xml` — **522** payload blocks, **3,114,377** bytes, SHA-256 `b2217224888fb9b6de664ae816dcee8748507c37e9da0f2d259cc676cd4105a4`. Its complete bytes were previewed on this instance from a recorded zero-state (CR2 F06) with 0
...
**CORRECTED 2026-09-08 — gate 7 is MET on the artifact that ships now, so the current rollup is 6 pass · 1
pass-with-one-native-step · 0 NOT MET.** The consolidated 522-block export
...
On the consolidated 522-block export that ships now, gate 7 / AAP §0.7.1 is **MET on its own bytes**: the
...
| 7. Update Set — **CORRECTED 2026-09-08** | Loads/previews with zero errors | ✅ **MET on the shipping 522-block export** | ✅ **MET** on the shipping 522-block export `b2217224…`: 0 `type=error` and 0 `type=warning` from a recorded zero-state (CR2 F06), then one native commit, 2026-09-08 — by a same-instance reset-and-reimport rather than an independent second PDI. The row above is retained as written and applies to the superseded packages. [`refine-run/CONSOLIDATION-FINAL-REPORT.md`](./refine-run/CONSOLIDATION-FINAL-REPORT.md) |
...
**CORRECTED 2026-09-08 — the current live suite result is `TES0001006`, taken on the bytes that ship.** The
...
**CORRECTED 2026-09-08 — the standing figure is now `TES0001006`, taken on the bytes that ship.** The
...
>
> **CORRECTED 2026-09-08 — a package-alone figure for the bytes that ship now exists: take `TES0001006`.** On
...
commit on 2026-09-08, nothing run in between — so the full gate now belongs to the bytes that ship:
`update-set/x_casemgmt_case_management_update_set.xml`, **522 blocks / 3,114,377 bytes / SHA-256
`b2217224888fb9b6de664ae816dcee8748507c37e9da0f2d259cc676cd4105a4`**. Item 2's ATF half was taken on that same
...
> **CORRECTED 2026-09-08 — the gate has since been run on the bytes that ship, and Path A no longer exists.**
...
> one native commit, nothing running in between. AAP §0.7.1 and the Update Set gate are therefore **MET** on
> the 522-block export `b2217224…`, by a same-instance reset-and-reimport rather than an independent second
> PDI. Both candidate packages were superseded and **deleted**, so there is no promotion left to make. Full
...
**CORRECTED 2026-09-08 — the suite HAS since been re-run on the bytes that ship, so the newer figure is the one
to quote.** Against the consolidated 522-block export (`b2217224…`), committed once natively onto an instance
