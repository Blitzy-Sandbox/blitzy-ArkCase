...
│                                      (926 blocks · 3,780,373 bytes · SHA-256 a9204411…), byte-identical
...
│                                      SHA-256 e109e1d1…) and is the available upgrade path — see item 4
...
│                                      deliverable (a9204411…) carries 3 sys_db_object + 25
...
- **Update Set XML:** `servicenow-case-management-poc/update-set/x_casemgmt_case_management_update_set.xml` —
  **the ELECTED deliverable: the retained original package, re-cut 2026-09-03 with the native choice
  ...
  bytes, SHA-256 `e109e1d107e28401cbcc74a7e0006f10cfa68d668560843d6e0fee6f8b79408d`) and is the available
...
- **Identity:** `update-set/x_casemgmt_case_management_update_set.xml` — **the ELECTED deliverable: 926 update
  blocks, 3,780,373 bytes, SHA-256
  `a9204411593a4811f30540d30c8d56d73d8c34e2a288a3ac541596a15aaec274`**, the retained original package as re-cut
  on 2026-09-03 with seven platform-native choice composites (superseding `7292a6fe…`, 919 of the 926 children
  unchanged),
  ...
  on these bytes: no preview of the complete file was ever run on them.** What was run, on the seven choice
...
- **Round-trip status: zero *reference* problems proven on these bytes; zero problems *of any type* proven on
  ...
  **On the 988 records of the retained rebuilt package, the full trip has since been measured — on export 3's
  ...
  re-sequencing then changed those bytes to `90ee0249…`, and the 2026-09-03 choice-composite fix changed them
  again to the retained `e109e1d1…` file on disk; the trip on neither of those exact sequences has ever been
  run. Neither sequence is the elected deliverable, and neither result transfers to it.** The
  paragraph that follows records why the pass that produced the `7292a6fe…` bytes — the elected file as it
  stood before the 2026-09-03 fix — stopped
...
> the elected deliverable's complete byte sequence (`a9204411…`), so step 4's "confirmation that all validation
