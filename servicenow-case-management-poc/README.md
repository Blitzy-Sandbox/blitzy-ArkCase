...
>
> 1. **Two manual post-import steps *were* mandatory — the package that ships today no longer needs them.** The 2026-09-02 native-rebuild run committed the shipping package **once**, on a genuinely clean instance, and both the physical table schema (Defect C's storage half) and all **27** ACL role-link records (Defect 9) arrived **from the package alone**: three tables at HTTP 200 with dictionary rows 21 / 14 / 13, and 27 links split manager 14 / agent 10 / viewer 3 — with `scripts/post_import_remediation.js` never run and no second commit. What survives is Defect C's **choice half**: `sys_choice` rows are still absent for the three tables, so choice lists — like the seed-row linkage and `opened_date` — still need the documented post-commit step (`scripts/seed_demo_data.js`, and a decision on the choice payloads). Evidence, with the commit counters and post-commit queries: [`docs/refine-run/FINAL-REPORT.md`](docs/refine-run/FINAL-REPORT.md). The paragraphs below that describe the remediation route remain accurate for the retained original package (`update-set/x_casemgmt_case_management_update_set.FALLBACK.xml`).
...
>
> 4. **AAP §0.7.1's zero-preview-error gate is a binary hard gate, and for the byte sequence that ships it is NOT MET: the round trip on the exact bytes that ship has NOT been run and is owed. The deliverable is therefore BLOCKED from hand-over — do not treat it as verified, shippable or ready to deliver, and do not read this as a partial or qualified result.** The file at `update-set/x_casemgmt_case_management_update_set.xml` is the §0.5.2-reordered native-rebuild package — **988 blocks, 4,062,436 bytes, SHA-256 `90ee024968f29a36f420eeeea908676054bc0d79067ff8d26e826662d78d35d7`** — and that is the digest to verify a copy of the artifact against, not a digest that has been round-tripped. **The election between running the owed gate on those bytes (Path A) and invoking the retained fallback (Path B) is a human decision that this run does not make; both paths are stated with their measured costs in [`docs/PDI_LIMITATIONS_AND_KNOWN_ISSUES.md`](docs/PDI_LIMITATIONS_AND_KNOWN_ISSUES.md) §10.0, alongside the re-opened item 1a.** The gate was MET on the pre-reorder sequence, and that is what the following records. Those 988 payload records were previewed on 2026-09-02 against an instance holding none of the three tables, to **0 `type=error` and 0 `type=warning`** problems with nothing skipped or ignored, then committed by the native UI action: "Succeeded 100%", 613 inserted / 375 updated / 0 collisions / 988 total, no partial apply — all of it measured on the **pre-reorder, previewed-and-committed byte sequence, 988 blocks / 4,062,436 bytes / SHA-256 `eee9fabd91fb5dfe94657c22e71a4cfa448c46e4dc7d35189ed6bb6361e4d4ae`, `2026-09-02T20:53:14Z`**. The file was afterwards re-sequenced into the AAP §0.5.2 dependency order, and that changed the order of the `<sys_update_xml>` blocks and nothing else: identical header and tail bytes, the same 988 record bodies and identities byte-for-byte, the same 4,062,436 bytes, the same 44-class census, and a block multiset provably identical to the previewed bytes. **Under this run's frozen rule — a package that changes after its verification has a stale recorded checksum and re-runs the whole gate before it is ship-ready — the checksum is stale and the exact-byte round trip on `90ee0249…` is outstanding: AAP §0.7.1 is satisfied for the previewed sequence and owed for the sequence that ships.** What closes it: on a genuinely clean PDI, run [`docs/HUMAN_DEPLOYMENT_RECREATE_GUIDE.md`](docs/HUMAN_DEPLOYMENT_RECREATE_GUIDE.md) §5 against the `90ee0249…` file — upload, assert 988 children, preview to zero `type=error`, commit through the native "Commit Update Set" UI action, confirm physical storage and all 27 role links — then record `90ee0249…` as verified with that run's timestamp. Which package is handed over is **not** settled here: reverting the reorder would re-open a HIGH AAP §0.5.2 violation and the fallback was never previewed at all, so neither path is free — the election is the human decision recorded in §10.0, and until it is made and executed this gate stays NOT MET and the deliverable stays BLOCKED. **The re-sequencing was verified statically, and static verification is corroboration rather than the gate:** `xmllint --noout` clean, 988 blocks counted, multiset identity proven against the previewed bytes, and every §0.5.2 dependency assertion passing (application record first, tables before dictionary rows, dictionary before documentation and choices, roles before ACLs, both before the 27 role links, subflows before the two state-machine flows, reports before both dashboards, portal → page → widget → container → row → column → instance, ATF test → step → step-input value, and all 28 seed rows last). The reordered bytes were **not** themselves re-uploaded, re-previewed or re-committed on any instance. See [`docs/refine-run/FINAL-REPORT.md`](docs/refine-run/FINAL-REPORT.md). The rest of this item is the historical record of the **earlier** revisions, ending in the retained original now kept as `update-set/x_casemgmt_case_management_update_set.FALLBACK.xml`: three separate results exist and they belong to three different revisions of this file, which is exactly the distinction earlier revisions of this paragraph collapsed. **Zero preview problems of any type, then `state=committed`**, was measured on the **913-block, 3,618,378-byte, SHA-256 `7272edfc…`** revision after a proven teardown — progression **41 → 298 → 0**. **Zero `Could not find a record` problems** (the package-intrinsic reference class, 63 → 0) was measured on the **925-block, 3,698,577-byte, `e49a7654…`** revision, previewed against an already-populated instance that left 31 `Found a local update that is newer than this one` collisions — every one of them confirmed to be that instance's own history — with **commit withheld** because the instance is shared. **The 926-block, 3,781,097-byte, SHA-256 `7292a6fe30413a9fb0b115e160c668edb7487b4391865b21a011a7be1add66b7` bytes — the retained FALLBACK package, which shipped before the native rebuild — were never previewed.** What *was* measured on them: each of the 13 re-synced payloads and the 1 added block was applied to the live instance and read back field-for-field identical to its artifact, and every table and column they name was confirmed to exist. [`docs/PDI_LIMITATIONS_AND_KNOWN_ISSUES.md` §0.2, §0.3b and §0.3c](docs/PDI_LIMITATIONS_AND_KNOWN_ISSUES.md) hold the full record of that lineage; the trip that §10.0 item 1a tracks as open work was **executed on the pre-reorder `eee9fabd…` records on 2026-09-02 and is re-opened for the `90ee0249…` bytes that ship** (`docs/refine-run/`, and §10.0 item 1a of the limitations register for exactly what remains).
>
> Every one of these is measured, not estimated — and every measurement in this deliverable is stated as of the date it was taken. **The instance those measurements were taken on has been hibernating since 2026-08-11**, serving ServiceNow's placeholder page on every route, so nothing in that set was re-measured on it. A **newly provisioned validation PDI** was used on 2026-09-02 instead, and the deliverable's preview, commit, post-commit state and ATF suite were all re-measured there — [`docs/refine-run/FINAL-REPORT.md`](docs/refine-run/FINAL-REPORT.md); [`docs/PDI_LIMITATIONS_AND_KNOWN_ISSUES.md` §0.11](docs/PDI_LIMITATIONS_AND_KNOWN_ISSUES.md) records what that leaves unproven and what has to happen first. [`docs/PDI_LIMITATIONS_AND_KNOWN_ISSUES.md` §0](docs/PDI_LIMITATIONS_AND_KNOWN_ISSUES.md) is the authoritative current-state record and the place to start.
...
5. **Single Update Set deliverable** — the final scoped application is exported as one XML at `update-set/x_casemgmt_case_management_update_set.xml`. **The single-file constraint is met; AAP §0.7.1's gate is not. That gate is binary and it is NOT MET for the byte sequence that ships, so the artifact is not cleared for hand-over — see item 4 at the top of this file and §10.0 of [`docs/PDI_LIMITATIONS_AND_KNOWN_ISSUES.md`](docs/PDI_LIMITATIONS_AND_KNOWN_ISSUES.md) for the election that closes it. It is proven in full on the earlier `7272edfc…` revision (913 blocks / 3,618,378 bytes), which was taken through a complete teardown → upload → preview → commit run reaching 0 problems of any type; it is proven for the reference class only on the immediately preceding `e49a7654…` revision (925 blocks / 3,698,577 bytes)** — previewed against an already-populated instance, that file yielded 31 problems, all of them `Found a local update that is newer than this one` (this instance's own change history, impossible on a fresh PDI) and **zero** `Could not find a record` problems, with commit withheld because the verification instance is shared. **Superseded on 2026-09-02 by the native rebuild: the sentence that stood here — "on the 926-block bytes that ship today, no preview has been run at all" — described the 926-block package now retained as the FALLBACK, which is no longer the deliverable. What ships today is the 988-block, §0.5.2-reordered `90ee0249…` file, and the position on it is the one item 4 states: the 988 records were previewed to zero problems of any type and committed on the pre-reorder, previewed-and-committed `eee9fabd…` byte sequence, while the round trip on the exact `90ee0249…` byte sequence has not been run and is owed.** See [`docs/validation-gates.md`](docs/validation-gates.md) Gate 7 and [`docs/PDI_LIMITATIONS_AND_KNOWN_ISSUES.md`](docs/PDI_LIMITATIONS_AND_KNOWN_ISSUES.md) §0.3c and §10.0 item 1a.
...
- **Update Set XML:** `servicenow-case-management-poc/update-set/x_casemgmt_case_management_update_set.xml` —
  the §0.5.2-reordered native-rebuild package, SHA-256 `90ee0249…`. **Its hand-over is BLOCKED: the AAP
  §0.7.1 Update Set gate is binary and it is NOT MET on these bytes** (item 4 of the note at the top of this
  file; the Path A / Path B election is in
  [`docs/PDI_LIMITATIONS_AND_KNOWN_ISSUES.md`](docs/PDI_LIMITATIONS_AND_KNOWN_ISSUES.md) §10.0).
...
- **Identity:** `update-set/x_casemgmt_case_management_update_set.xml` — **988 update blocks, 4,062,436 bytes,
  SHA-256 `90ee024968f29a36f420eeeea908676054bc0d79067ff8d26e826662d78d35d7`**, the §0.5.2-reordered
  native-rebuild package. **Its shipping status is BLOCKED: the AAP §0.7.1 Update Set gate is a binary gate and
  it is NOT MET on these bytes, so this artifact is not verified, not shippable and not ready to hand over until
  the owed run is performed.** `90ee0249…` is the digest to verify a copy of the file against — nothing more.
  Which package is handed over is a human election, Path A or Path B, recorded in §10.0 of
  [`docs/PDI_LIMITATIONS_AND_KNOWN_ISSUES.md`](docs/PDI_LIMITATIONS_AND_KNOWN_ISSUES.md).
  **Do not read the digest as a verified-by-round-trip digest.** The preview
  and commit of 2026-09-02 ([`docs/refine-run/FINAL-REPORT.md`](docs/refine-run/FINAL-REPORT.md)) were measured
  on the **pre-reorder, previewed-and-committed byte sequence, SHA-256
  `eee9fabd91fb5dfe94657c22e71a4cfa448c46e4dc7d35189ed6bb6361e4d4ae`** (the same 988 records, the same
  4,062,436 bytes); the file was then re-sequenced into AAP §0.5.2 dependency order — a different block order
  and therefore a different digest — and was checked statically rather than by a second round trip.
  **The round trip on the `90ee0249…` bytes is owed and has not been run** (see the package note at the top of
  this file, and §10.0 item 1a of
  [`docs/PDI_LIMITATIONS_AND_KNOWN_ISSUES.md`](docs/PDI_LIMITATIONS_AND_KNOWN_ISSUES.md)).
  Quote those numbers and no others. The revision immediately before it — **926 blocks, 3,781,097 bytes,
  `7292a6fe30413a9fb0b115e160c668edb7487b4391865b21a011a7be1add66b7`** — is retained verbatim as
  `update-set/x_casemgmt_case_management_update_set.FALLBACK.xml`, and it is that file the measurements in the
  rest of this section describe; earlier revisions carried different numbers again — 925 blocks / 3,698,577 bytes / `e49a7654…` immediately
...
- **Round-trip status: zero *reference* problems proven on these bytes; zero problems *of any type* proven on
  ...
  **On the 988 records that ship, the full trip has since been measured — on the pre-reorder,
  previewed-and-committed byte sequence `eee9fabd…` (988 blocks / 4,062,436 bytes): 0 `type=error` and
  ...
  re-sequencing then changed the file's bytes to `90ee0249…`, and the trip on that exact sequence is owed and
  unperformed.** The
  paragraph that follows records why the pass that produced the retained `7292a6fe…` fallback bytes stopped
...
- **The ATF suite is green: 20 / 20 tests Success and 180 / 180 step results Success**, zero failures, errors or
  skips, in roughly 4 minutes, leaving no test residue behind — and that rollup has been **reproduced twice
  independently** (runs `TES0001016` and `TES0001017`, 2026-08-10, the second dispatched through the product UI
  with a browser runner attached). Quote the rollup and the measurement method, **not a `TES…` identifier**:
  `sys_atf_test_suite_result` rows are not durable on this shared instance, and the two rows earlier revisions of
  this README cited as current — `TES0001015` and `TES0001014` — no longer resolve on it. (An earlier *series* of
  runs, `TES0001010`–`TES0001012`, scored **16 / 4** — `ATF 07` plus the three form tests `ATF 15` / `ATF 16` /
  `ATF 17`; that result predates the fixes and is history, not status. `TES0001014` was the last verdict taken
  against a fresh re-load of the shipped `atf/*.xml` artifacts — the project's only serialized-import proof, on an
  earlier package revision; repeating it on the shipping bytes is open work, tracked as §10.0 item 2 of the
...
> the exact shipping byte sequence, so step 4's "confirmation that all validation gates passed" cannot be
> given for the file in your hands: the Update Set gate is binary and it is NOT MET on those bytes (item 4 of
> the note at the top of this file). Running step 2 on them is what makes this deliverable deliverable.**
> The 2026-09-02 native-rebuild run committed those 988 records once on a clean
> instance — on the pre-reorder, previewed-and-committed `eee9fabd…` sequence — and
> got physical storage for all three tables and all 27 ACL role links out of the commit itself — the two
> paragraphs below describe the older package, retained as `…FALLBACK.xml`, where a bare commit did leave the
> tables without storage and the ACLs without their role links. The one item that still needs a post-commit
> step on today's bytes is the **choice lists** (`sys_choice` rows are absent for the three tables), along with
> the seed-row linkage and `opened_date`; see [`docs/refine-run/FINAL-REPORT.md`](docs/refine-run/FINAL-REPORT.md).
