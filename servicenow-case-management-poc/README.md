...
- **Identity:** `update-set/x_casemgmt_case_management_update_set.xml` — **988 update blocks, 4,062,436 bytes,
  ...
  whose 988 records were verified by preview and commit on 2026-09-02
  ([`docs/refine-run/FINAL-REPORT.md`](docs/refine-run/FINAL-REPORT.md)) and which was then re-sequenced into
  AAP §0.5.2 dependency order — the same 988 records byte-for-byte, the same byte count, a different block
  order and therefore a different digest, checked statically rather than by a second round trip (see the
  package note at the top of this file).
...
- **Round-trip status: zero *reference* problems proven on these bytes; zero problems *of any type* proven on
  ...
  **On the 988 records that ship (988 blocks / 4,062,436 bytes; the file hashes to `90ee0249…` since the
  §0.5.2 re-sequencing, which changed block order only) the full trip has since been measured:
  0 `type=error` and 0 `type=warning` preview problems on a genuinely clean instance, then a single UI-action
  commit that succeeded 100% — see [`docs/refine-run/FINAL-REPORT.md`](docs/refine-run/FINAL-REPORT.md).** The
...
> they are sufficient.** The 2026-09-02 native-rebuild run committed these bytes once on a clean instance and
