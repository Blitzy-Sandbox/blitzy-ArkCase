...
ArkCase is a case management platform built on top of mature open source services. A typical deployment integrates ArkCase with Solr (search), ActiveMQ (messaging), MySQL (relational store), Alfresco (content repository), and Pentaho (reporting). The codebase is a multi-module Maven project organized around a core API, pluggable services, tool integrations, and a web user interface.
